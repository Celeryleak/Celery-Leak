...
namespace EyeStepPackage
{
  public class EyeStep
  {
    ...
    public static EyeStep.inst read(ulong handle, ulong address)
    {
      EyeStep.inst p = new EyeStep.inst();
      p.address = address;
      int lpNumberOfBytesRead = 0;
      Imports.ReadProcessMemory(handle, address, p.bytes, 16, ref lpNumberOfBytesRead);
      for (int index1 = 0; index1 < EyeStep.OP_TABLE.Length; ++index1)
      {
        ...
        bool flag2 = (int) p.bytes[p.len] == (int) num1;
        bool flag3 = false;
        for (int index2 = 2; index2 < 11 && opInfo.code.Length > index2; index2 += 3)
        {
          if (opInfo.code[index2] == '+')
          {
            ...
            {
              flag3 = true;
              flag2 = (int) p.bytes[p.len] >= (int) num1 && (int) p.bytes[p.len] < (int) num1 + 8;
              break;
            }
            if (opInfo.code[index2 + 1] == 'm' & flag2)
            {
              ...
              {
                flag2 = EyeStep.longreg(p.bytes[p.len + 1]) == (int) num2;
                break;
              }
              byte num3 = (byte) ((uint) num2 - 8U);
              flag2 = EyeStep.longreg(p.bytes[p.len + 1]) == (int) num3 && p.bytes[p.len + 1] >= (byte) 192;
              break;
            }
            if (flag2)
            {
              ...
              flag2 = (int) p.bytes[p.len] == (int) num1;
            }
          }
        }
        if (flag2)
            ...
            case 0:
              ...
              for (int c = 0; c < length; c++)
              {
                ...
                if (num4 == byte.MaxValue)
                  reg_type1 = (byte) EyeStep.longreg(p.bytes[p.len]);
                if (flag3)
                  reg_type1 = (byte) EyeStep.finalreg(p.bytes[p.len - 1]);
                switch (p.operands[c].opmode)
                {
                  case OP_TYPES.AL:
                    int num5 = (int) p.operands[c].append_reg((byte) 0);
                    p.data += "al";
                    p.operands[c].flags |= 1024U;
                    break;
                  case OP_TYPES.AH:
                    int num6 = (int) p.operands[c].append_reg((byte) 4);
                    p.data += "ah";
                    p.operands[c].flags |= 1024U;
                    break;
                  case OP_TYPES.AX:
                    int num7 = (int) p.operands[c].append_reg((byte) 0);
                    p.data += "ax";
                    p.operands[c].flags |= 2048U;
                    break;
                  case OP_TYPES.EAX:
                    int num8 = (int) p.operands[c].append_reg((byte) 0);
                    p.data += "eax";
                    p.operands[c].flags |= 4096U;
                    break;
                  case OP_TYPES.ECX:
                    int num9 = (int) p.operands[c].append_reg((byte) 1);
                    p.data += "ecx";
                    p.operands[c].flags |= 4096U;
                    break;
                  case OP_TYPES.EBP:
                    int num10 = (int) p.operands[c].append_reg((byte) 3);
                    p.data += "ebp";
                    p.operands[c].flags |= 4096U;
                    break;
                  case OP_TYPES.CL:
                    int num11 = (int) p.operands[c].append_reg((byte) 1);
                    p.data += "cl";
                    p.operands[c].flags |= 1024U;
                    break;
                  case OP_TYPES.Sreg:
                    p.data += EyeStep.mnemonics.sreg_names[(int) p.operands[c].append_reg(reg_type1)];
                    p.operands[c].flags |= 131072U;
                    break;
                  ...
                  case OP_TYPES.GS:
                    ...
                    break;
                  case OP_TYPES.one:
                    p.operands[c].disp32 = (uint) (p.operands[c].disp16 = (ushort) (p.operands[c].disp8 = (byte) 1));
                    p.data += "1";
                    break;
                  case OP_TYPES.r8:
                    p.data += EyeStep.mnemonics.r8_names[(int) p.operands[c].append_reg(reg_type1)];
                    p.operands[c].flags |= 1024U;
                    break;
                  case OP_TYPES.r16:
                    p.data += EyeStep.mnemonics.r16_names[(int) p.operands[c].append_reg(reg_type1)];
                    p.operands[c].flags |= 2048U;
                    break;
                  case OP_TYPES.r16_32:
                  case OP_TYPES.r32:
                    p.data += EyeStep.mnemonics.r32_names[(int) p.operands[c].append_reg(reg_type1)];
                    p.operands[c].flags |= 4096U;
                    break;
                  case OP_TYPES.r64:
                    p.data += EyeStep.mnemonics.r64_names[(int) p.operands[c].append_reg(reg_type1)];
                    p.operands[c].flags |= 8192U;
                    break;
                  case OP_TYPES.r_m8:
                  case OP_TYPES.r_m16:
                  ...
                  case OP_TYPES.STi:
                    if (((int) p.flags & 32) == 32)
                      p.data += "cs:";
                    else if (((int) p.flags & 128) == 128)
                      p.data += "ds:";
                    else if (((int) p.flags & 256) == 256)
                      p.data += "es:";
                    else if (((int) p.flags & 64) == 64)
                      p.data += "ss:";
                    else if (((int) p.flags & 512) == 512)
                      p.data += "fs:";
                    else if (((int) p.flags & 1024) == 1024)
                      p.data += "gs:";
                    if (p.operands[c].opmode == OP_TYPES.moffs16_32)
                    {
                      p.data += "[";
                    ...
                    if (c == 0)
                      num4 = reg_type1;
                    byte reg_type2 = (byte) EyeStep.finalreg(p.bytes[p.len]);
                    switch ((int) p.bytes[p.len] / 64)
                    {
                      case 0:
                        p.data += "[";
                        switch (reg_type2)
                        {
                          ...
                          case 5:
                            EyeStep.inst inst = p;
                            string data = inst.data;
                            uint num12;
                            p.operands[c].disp32 = num12 = BitConverter.ToUInt32(p.bytes, p.len + 1);
                            num12 = num12;
                            string str = num12.ToString("X8");
                            inst.data = data + str;
                            p.operands[c].flags |= 512U;
                            p.len += 4;
                            break;
                          default:
                            p.data += EyeStep.mnemonics.r32_names[(int) p.operands[c].append_reg(reg_type2)];
                            p.operands[c].flags |= 4096U;
                            break;
                        }
                        p.data += "]";
                      ...
                      case 2:
                        ...
                        break;
                      case 3:
                        OP_TYPES opmode = p.operands[c].opmode;
                        if ((uint) opmode <= 38U)
                        {
                          if ((uint) opmode <= 28U)
                          {
                          ...
                          p.data += EyeStep.mnemonics.r8_names[(int) p.operands[c].append_reg(reg_type2)];
                          p.operands[c].flags |= 1024U;
                          break;
label_93:
                          p.data += EyeStep.mnemonics.r16_names[(int) p.operands[c].append_reg(reg_type2)];
                          p.operands[c].flags |= 2048U;
                          break;
                        }
                        if ((uint) opmode <= 69U)
                        {
                          switch (opmode)
                          {
                            ...
                            case OP_TYPES.xmm_m64:
                            case OP_TYPES.xmm_m128:
                              p.data += EyeStep.mnemonics.xmm_names[(int) p.operands[c].append_reg(reg_type2)];
                              p.operands[c].flags |= 16384U;
                              goto label_113;
                            case OP_TYPES.mm_m64:
                              p.data += EyeStep.mnemonics.mm_names[(int) p.operands[c].append_reg(reg_type2)];
                              p.operands[c].flags |= 32768U;
                              goto label_113;
                            case OP_TYPES.STi:
                            case OP_TYPES.ST:
                              p.data += EyeStep.mnemonics.st_names[(int) p.operands[c].append_reg(reg_type2)];
                              p.operands[c].flags |= 65536U;
                              goto label_113;
                          }
                        }
                        else
                        {
                          ...
                          {
                            case OP_TYPES.CRn:
                              p.data += EyeStep.mnemonics.cr_names[(int) p.operands[c].append_reg(reg_type2)];
                              p.operands[c].flags |= 524288U;
                              goto label_113;
                            case OP_TYPES.DRn:
                              p.data += EyeStep.mnemonics.dr_names[(int) p.operands[c].append_reg(reg_type2)];
                              p.operands[c].flags |= 262144U;
                              goto label_113;
                          }
                        }
label_99:
...
label_113:
                  ...
                  case OP_TYPES.imm32:
                    ...
                    break;
                  case OP_TYPES.mm:
                    p.data += EyeStep.mnemonics.mm_names[(int) p.operands[c].append_reg(reg_type1)];
                    p.operands[c].flags |= 32768U;
                    break;
                  case OP_TYPES.xmm:
                    p.data += EyeStep.mnemonics.xmm_names[(int) p.operands[c].append_reg(reg_type1)];
                    p.operands[c].flags |= 16384U;
                    break;
                  case OP_TYPES.xmm0:
                    int num13 = (int) p.operands[c].append_reg((byte) 0);
                    p.data += "xmm0";
                    p.operands[c].flags |= 16384U;
                    break;
                  case OP_TYPES.ST:
                    p.data += EyeStep.mnemonics.st_names[(int) p.operands[c].append_reg((byte) 0)];
                    p.operands[c].flags |= 65536U;
                    break;
                  case OP_TYPES.CRn:
                    p.data += EyeStep.mnemonics.cr_names[(int) p.operands[c].append_reg(reg_type1)];
                    p.operands[c].flags |= 524288U;
                    break;
                  case OP_TYPES.DRn:
                    p.data += EyeStep.mnemonics.dr_names[(int) p.operands[c].append_reg(reg_type1)];
                    p.operands[c].flags |= 262144U;
                    break;
                }
                ...
                void get_imm8(byte x, bool constant)
                {
                  string str;
                  if (!constant)
                  {
                    p.operands[c].imm8 = x;
                    p.operands[c].flags |= 16U;
                    str = x <= (byte) 127 ? "+" + p.operands[c].imm8.ToString("X2") : "-" + (256 - (int) p.operands[c].imm8).ToString("X2");
                  }
                  else
                  {
                    p.operands[c].disp8 = x;
                    p.operands[c].flags |= 128U;
                    str = p.operands[c].disp8.ToString("X2");
                  }
                  p.data += str;
                  ++p.len;
                }
                ...
                void get_imm16(ushort x, bool constant)
                {
                  ...
                  if (!constant)
                  {
                    ...
                    p.operands[c].flags |= 32U;
                    str = x <= (ushort) short.MaxValue ? "+" + p.operands[c].imm16.ToString("X4") : "-" + (65536 - (int) p.operands[c].imm16).ToString("X4");
                  }
                  else
                  {
                    p.operands[c].disp16 = x;
                    p.operands[c].flags |= 256U;
                    str = p.operands[c].disp16.ToString("X4");
                  }
                  p.data += str;
                  p.len += 2;
                }
                ...
                void get_imm32(uint x, bool constant)
                {
                  ...
                  if (!constant)
                  {
                    ...
                    p.operands[c].flags |= 64U;
                    str = x <= (uint) int.MaxValue ? "+" + p.operands[c].imm32.ToString("X8") : "-" + ((uint) -(int) p.operands[c].imm32).ToString("X8");
                  }
                  else
                  {
                    p.operands[c].disp32 = x;
                    p.operands[c].flags |= 512U;
                    str = p.operands[c].disp32.ToString("X8");
                  }
                  p.data += str;
                  p.len += 4;
                }
                ...
                {
                  byte x = p.bytes[++p.len];
                  byte reg_type1 = (byte) EyeStep.longreg(x);
                  byte reg_type2 = (byte) EyeStep.finalreg(x);
                  ...
                  {
                    if (reg_type2 == (byte) 5 && p.bytes[p.len - 1] < (byte) 64)
                    {
                      p.data += EyeStep.mnemonics.r32_names[(int) p.operands[c].append_reg(reg_type1)];
                      p.operands[c].flags |= 4096U;
                    }
                    ...
                    if ((int) x / 64 > 0)
                    {
                      p.operands[c].mul = EyeStep.multipliers[(int) x / 64];
                      string str = p.operands[c].mul.ToString("X1");
                      p.data += "*";
                      p.data += str;
                    }
                  }
                ...
                {
                  ulong num = p.address + (ulong) (uint) p.len;
                  p.operands[c].rel8 = x;
                  p.data += (num + 1UL + (ulong) p.operands[c].rel8).ToString("X8");
                  ++p.len;
                }
                ...
                void get_rel16(ushort x)
                {
                  ...
                  p.data += (num + 2UL + (ulong) p.operands[c].rel16).ToString("X8");
                  p.len += 2;
                }
                ...
                void get_rel32(uint x)
                {
                  ...
                  p.data += (num + 4UL + (ulong) p.operands[c].rel32).ToString("X8");
                  p.len += 4;
                }
              }
              goto label_129;
...
label_129:
    ...
    public static List<EyeStep.inst> read(ulong handle, ulong address, uint count)
    {
      ulong address1 = address;
      List<EyeStep.inst> instList = new List<EyeStep.inst>();
      for (uint index = 0; index < count; ++index)
      {
        EyeStep.inst inst = EyeStep.read(handle, address1);
        instList.Add(inst);
        address1 += (ulong) (uint) inst.len;
      }
      return instList;
    }

    public static List<EyeStep.inst> read_range(ulong handle, ulong from, ulong to)
    {
      ulong address = from;
      List<EyeStep.inst> instList = new List<EyeStep.inst>();
      EyeStep.inst inst;
      for (; address < to; address += (ulong) (uint) inst.len)
      {
        inst = EyeStep.read(handle, address);
        instList.Add(inst);
      }
      return instList;
    }
